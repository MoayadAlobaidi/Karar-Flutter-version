...
| `modules/` | 29 module directories, each behind a `public-api.ts`; 19 have code, the rest are skeletons awaiting their phase |
...
modules/     29 module directories, each with public-api.ts and MODULE.md; 19 have code
...
- **Runtime contract conformance** — since Phase 4, real serialized responses from the composed application validated against the OpenAPI document that describes them, with ledgers asserted empty: no response carries an RFC 7807 body under the wrong media type, and no operation describes its body in prose without a schema. The merged contract declares **273 operation/status pairs across 55 paths**; two suites cover **148** of them — 82 of the 128 non-financial pairs, and 66 of the 145 the Phase 5 financial surface declares, the financial ones in their own file so that one failure does not hide the rest. The contract drift check binds contract to client; this binds server to contract ([`flutter.md` §8](docs/architecture/flutter.md)).
