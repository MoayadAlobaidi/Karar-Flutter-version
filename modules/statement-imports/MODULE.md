...
The first four of those are rows `modules/transactions` owns, and the freshness observation is a row `modules/financial-connections` owns. **Each is written by the module that owns it, on this transaction** — `PrismaStatementCommitWriter` satisfying `ImportedRecordCommitPort`, and `PrismaSourceObservationWriter` satisfying `SourceObservationWriterPort`, both declared by the owning module and both taking the open transaction as a parameter rather than opening one. So the unit of work is one transaction spanning three modules, with each module writing only its own tables. `__tests__/module-boundary.test.ts` scans this module's source and fails on any write to any of those five tables, with no exception list.

**The freshness observation is transactional, not eventual, and the direction of the risk decides it.** A `last_successful_import_at` that lags a commit is a report behind the facts; one written for a commit that rolled back is a claim this platform imported a statement it did not. Only the second is false, and the only way to make it unwritable is to put the observation in the transaction whose success it describes. It is still best-effort about the LINK: zero matching links is the ordinary answer for somebody who imported a file without ever linking its source, the count is ignored here, and nothing about an import fails because the route it arrived by is gone.
...
| `StatementCommitPort` | the atomic unit of work — everything, or nothing | `PrismaStatementCommitUnitOfWork` in `infrastructure/persistence`. It opens the ONE transaction and writes only this module's own rows in it; inside that same transaction the canonical records are written by `modules/transactions` and the source-link freshness by `modules/financial-connections` (see 'Dependencies') |
...
**Three ports this module CONSUMES rather than declares**, each declared by the module that owns what it touches and exported from that module's `public-api.ts`: `DedupFingerprintPort` and `ImportedRecordCommitPort` from `@karar/transactions`, and `SourceObservationWriterPort` from `@karar/financial-connections`. None is restated here — a second declaration of the same contract is how two definitions of "the same transaction" begin, and a second declaration of what may be written to a source link is how a delivery starts deciding things about a link.
...
Cross-module dependencies resolve through `public-api.ts` only. This module imports `@karar/transactions` (the dedup fingerprint port, the occurrence model, the HSF seam its canonical narrative is encrypted under, and `ImportedRecordCommitPort` with the `PrismaStatementCommitWriter` that fills it), `@karar/financial-accounts` (account access) and `@karar/financial-connections` (`SourceObservationWriterPort` with the `PrismaSourceObservationWriter` that fills it, for the source link a committed import refreshes). **No module imports this one**, which is why both cross-module writers live on the other side of the dependency rather than the port: neither of those modules can import this one without making a cycle, so each declares the contract for the rows it owns and this module calls it.
...
**No transport, no controller, deliberately — and that is still true of THIS MODULE.** The HTTP surface exists and is mounted, in `apps/api/src/financial/statement-imports.controller.ts`, where every controller lives; what this module contains is use cases and ports. The sentence here used to say mounting one was "a separate, later commit", which it was when written and is not any more. Architecture test 24 (resource limits) activates at phase 5, and the `phase5-ingestion-not-mounted-early` control fails the build on a controller that appears while the registry still reads an earlier phase. The route, its limit-policy registration and the phase move landed in **one** commit, so no path became reachable with its bounds unenforced by the suite.

**The commit no longer writes another module's tables — that debt is closed.** This module used to write `public.transactions`, `public.transaction_revisions`, `public.transaction_provenance` and `public.transaction_category_assignments` itself, because atomicity demanded one transaction and `PrismaTransactionRepository.commit` opens its own. The writer now lives where the rows do, as `PrismaStatementCommitWriter` in `modules/transactions/infrastructure/persistence`, satisfying `ImportedRecordCommitPort` **that module declares** — the port had to be declared there rather than here, unlike `PrismaFinancialRecordEraser` which aliases the accounts-side declaration, because this module already depends on that one and the reverse import would be a cycle. Atomicity survived the move intact: the port takes the open transaction, so `PrismaStatementCommitUnitOfWork` still opens exactly one and everything still lands together or not at all. `__tests__/module-boundary.test.ts` asserts the absence by scanning this module's own source for writes to those four tables.

**No cross-module write remains, and the last one to go was recorded here first.** The source-link freshness observation on `public.account_source_links` used to be an `updateMany` issued from this module's commit unit of work, and the boundary suite NAMED it as an exception rather than failing on it. It is now `SourceObservationWriterPort`, declared and implemented by `modules/financial-connections`, joining this module's single transaction through the same opaque handle the canonical records cross by. The exception is gone from the suite and `account_source_links` is in its forbidden set, so the shape cannot come back quietly — both of the old write shapes were reintroduced deliberately and each was confirmed to fail the suite before being removed again.

That move also fixed a defect the direct write had carried unnoticed: it never advanced `version`, and `account_source_links_guard` raises `KAR22` on any UPDATE that does not. Every link it actually matched would have aborted the whole import; it only ever appeared to work because no test had a link for it to match. The owning module's writer advances the token in the same statement, and `modules/financial-connections/__tests__/source-observation-writer.integration.test.ts` asserts it against a real link.
