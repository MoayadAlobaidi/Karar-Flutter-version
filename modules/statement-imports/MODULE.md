...
The first four of those are rows `modules/transactions` owns, and that module writes them — `PrismaStatementCommitWriter`, satisfying its own `ImportedRecordCommitPort`, which takes the open transaction as a parameter and joins it. So the unit of work is one transaction spanning two modules, with each module writing only its own tables. `__tests__/module-boundary.test.ts` scans this module's source and fails on any write to those four tables.
...
| `StatementCommitPort` | the atomic unit of work — everything, or nothing | `PrismaStatementCommitUnitOfWork` in `infrastructure/persistence`. It opens the ONE transaction and writes only this module's own rows in it; the canonical records inside that same transaction are written by `modules/transactions` (see 'Dependencies') |
...
Cross-module dependencies resolve through `public-api.ts` only. This module imports `@karar/transactions` (the dedup fingerprint port, the occurrence model, the HSF seam its canonical narrative is encrypted under, and `ImportedRecordCommitPort` with the `PrismaStatementCommitWriter` that fills it), `@karar/financial-accounts` (account access) and `@karar/financial-connections` (the source link a committed import refreshes). **No module imports this one**, which is why the commit writer lives on the other side of the dependency rather than the port: `modules/transactions` cannot import this module without making a cycle, so it declares the contract for the records it owns and this module calls it.
...
**One cross-module write remains, and it is deliberate:** the source-link freshness observation on `public.account_source_links`, which `modules/financial-connections` owns. It is best-effort by design — the link may not exist, and an import must not fail because the route it arrived through was deleted — so it is an `UPDATE` over zero or one row rather than a write that can create anything.
