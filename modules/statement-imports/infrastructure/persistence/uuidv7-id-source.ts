 ...
 *
 * v7 rather than v4 because these ids are primary keys, and this module
 * mints them in the largest batches in the codebase — one statement commit
 * produces four ids per row across four tables. The time-ordered prefix keeps
 * those index inserts local instead of scattering thousands of them across
 * the B-tree, and it does so without leaking anything a timestamp column does not
 ...
 * statement line the row is about — that lives only in the ciphertext and the
 * keyed fingerprint.
 */
