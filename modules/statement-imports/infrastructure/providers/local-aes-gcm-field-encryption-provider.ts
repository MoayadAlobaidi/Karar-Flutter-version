 ...
 * equivalent adapters in `modules/financial-accounts`,
 * `modules/transactions` and `modules/financial-connections`. Refusing to
 * construct is the guarantee; a comment saying "do not use in production" is
 * not.
 *
 ...
 *   also what makes the ciphertext non-deterministic, which is why duplicate
 *   detection is a keyed fingerprint and not a ciphertext compare;
...
/**
 * The domain-separation label. Distinct from every other module's on purpose:
 * even if they were all wired to one key, a ciphertext could not be replayed
 * from one module's table into another's — and the values at stake here are a
 * staged statement line's narrative and the source's own identifiers for the
 * subject, which sit beside the identical columns on `public.transactions`.
 */
const ASSOCIATED_DATA_LABEL = 'karar/statement-imports/hsf/v1';
 ...
 * two different subjects whose staged statement rows sit in the same table.
 */
...
export class LocalAesGcmFieldEncryptionProvider implements HsfFieldEncryptionPort {
  ...
  constructor(options: {
    ...
    this.#keyVersion = options.keyVersion ?? 'karar-ref:key-version:local-statement-imports-hsf@v1';
  }
  ...
  if (options.env !== 'local') {
    throw new HsfFieldEncryptionError(
      ...
      `no approved HSF field-encryption provider is wired for KARAR_ENV='${options.env}', and ` +
        'there is no fallback: statement_import_rows is HIGHLY_SENSITIVE_FINANCIAL and a staged ' +
        'row carries a statement narrative, a merchant, the source’s own transaction reference ' +
        'and an instrument mask — all of them only as ciphertext. Wire the environment’s ' +
