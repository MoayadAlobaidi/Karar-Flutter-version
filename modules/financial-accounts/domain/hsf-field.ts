 ...
 *
 * Three fields on an account are HSF, and each of them is a fact about a
 * person rather than an operational attribute of a row: the name the subject
 * gave the account, the institution label they typed when their bank is not
 * in the reviewed catalogue, and the masked fragment. "Al Bayt joint savings"
 * plus "Synthetic Unlisted Bank" plus a four-digit tail is an identification
 * of a person's banking relationships; the table's classification says so,
 * and until this type existed the columns behind it said otherwise.
 *
 * The split this type encodes is the one `modules/transactions` already
 * established for merchant, description and note: the domain holds PLAINTEXT
 * in memory, inside a wrapper that cannot leak through a log line;
 * encryption at rest is an INFRASTRUCTURE concern behind
 * `HsfFieldEncryptionPort`, and nothing in the domain knows about keys,
 * nonces, or algorithms. The domain still needs these as VALUES — an edit
 * compares old to new, the mask rule inspects characters, a list renders a
 * name — so "ciphertext the domain never sees" is not available.
 *
 * Reimplemented here rather than imported from the transactions module, for
 * the reason `uuidv7-id-source.ts` states about its own duplication: nothing
 * crosses a module boundary except through `public-api.ts`, and a shared
 * domain type would be a coupling neither module's boundary admits.
 *
 ...
 * the type is deliberately NOT a branded string — a branded string is still
 * a string, and `console.log(displayName)` would print it.
 */
 ...
 * unbounded ciphertext column, and so a field named for a NAME cannot become
 * a place to keep notes the classification does not cover. The database
 * carries the matching byte bound (migration 0088) — encryption preserves
...
export class HsfField {
   ...
   * deliberate: by the time a value reaches here the domain rules in
   * `financial-account.ts` have already turned every EXPECTED refusal (empty,
   * over-long, not a mask) into a `Result` arm the caller handles. Reaching
   * this constructor with a value those rules would reject is a defect in
   * this module, not an outcome a user produced.
   */
