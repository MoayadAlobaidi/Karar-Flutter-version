...
**What `CASCADE_DELETE` reaches, stated because it used to reach less than it claimed.** For `financial_accounts` the cascade is the account row, its balance snapshots (by foreign key), **and** every account-scoped row other modules hold: the account-source links in `modules/financial-connections` through `AccountSourceLinkEraserPort`, the payment instruments in `modules/payment-instruments` through `PaymentInstrumentEraserPort`, and the financial records in `modules/transactions` through `FinancialRecordEraserPort` — all three before the account row is removed, and the last of them taking the transfer matches in `modules/transfer-matching` with it. Until the Phase 5 remediation none of that happened at all — see 'Deletion deletes everything, and is not atomic' below.
...
| `PaymentInstrumentEraserPort` | erase every payment instrument spending from an account, returning the exact count | `modules/payment-instruments` |
...
`DeleteOwnAccount` used to claim it removed "everything scoped to" an account while removing the account and its snapshots only. Transactions, revisions, provenance and category assignments carry a raw `account_id` with no foreign key back here (no FK crosses a module boundary) and survived orphaned; so did the account-source links in `modules/financial-connections`, which additionally hold the encrypted external account reference for the source account — a protected external identity retained about a subject who had asked to be rid of it, and a live route by which the next import would have re-created the account just deleted; and so did the payment instruments in `modules/payment-instruments`, which say what SPENDS from the account and hold the encrypted mask a person reads off their own card, an instrument naming a deleted account being a way to spend from something the person believes no longer exists. All three are now erased — through `AccountSourceLinkEraserPort`, `PaymentInstrumentEraserPort` and `FinancialRecordEraserPort` — before the account row is deleted.

**A fourth table goes with them, one step further out.** `public.transfer_matches` in `modules/transfer-matching` names two of a person's transactions and says they were one movement of their own money. This module never reaches it directly and declares no port for it: `PrismaFinancialRecordEraser` cuts those relationships before deleting the records they name, through a port `modules/transactions` declares, and reports how many went in `financialRecordRelationshipsDeleted`. `AccountDeleted` carries that count so a person is told what actually went and not only the part this module has vocabulary for. `undefined` there means an eraser measured no relationships; `0` means it measured and found none, and the two are deliberately different claims.

**The order is source links, then payment instruments, then financial records, then the account row.** A source link is the route by which new records ARRIVE for an account and an instrument is a way to SPEND from it, which is the route by which new records are CAUSED: cut the routes first and nothing travels down them afterwards, whereas erasing the records first leaves a live link an import can use to write fresh rows in the gap before the account row goes — rows the delete would then orphan while reporting success. Stopping after the routes also loses less: an account that keeps every record but has lost its links and its instruments is coherent, retryable, and rebuildable by asking the subject again, where nobody can re-supply a year of transactions from memory.

**Links before instruments, and not the other way round.** The link is strictly the more urgent cut, because it is the only one of the two that can put a row in the database. An instrument cannot deliver anything by itself — no issuer named anywhere in this platform exposes an interface to it, and the instruments module's own status vocabulary answers `false` to "implies a live issuer link" for every value it permits — so a spend on a card becomes a record here only by arriving through a source link. Cutting the link closes the instrument's route too; cutting the instrument first would hold the one live inbound route open for longer to no purpose. Instruments still precede the records, by the rebuildability argument above and because an instrument outliving the records of what it spent is the more misleading residue of the two.

**Cross-module deletion is not atomic and this module does not claim it is.** Each eraser runs in its own module's principal-scoped transaction; the account delete runs in this module's, and no unit of work spans all four — one would require passing a live transaction handle through an application port. The residual window is narrowed by ordering (visibility and version are checked before anything is erased), closed on retry (every eraser is idempotent by contract), and reported honestly: a source-link refusal answers `source_link_erasure_incomplete` with nothing else attempted, an instrument refusal answers `instrument_erasure_incomplete` carrying the source links that already went, a record refusal answers `erasure_incomplete` carrying both counts that already went, and a successful erasure followed by a failed account delete answers `deletion_partially_applied`. None of them is ever reported as success.

**One argument for the instrument eraser is optional at the constructor, and the residual is stated rather than hidden.** Zero is the true answer for a deployment that does not compose `modules/payment-instruments` — those rows can only exist if that module wrote them — so a deployment without it is not made to hand in a stub that says so. The residual is a deployment that DOES compose the module and forgets to pass the eraser: it gets the default and silently reproduces the defect. Nothing in this module can catch that; it is a composition-root concern and belongs in that root's own wiring test. There is no composition root in this repository yet, and when one arrives the test arrives with it.

**No eraser's `reason` may carry store text outward.** Driver text can hold a connection string, the failing SQL, or a fragment of the row being erased; a refusal from this path therefore carries a stable message of this module's own and attaches the original throw NON-ENUMERABLY for the one boundary allowed to log it (`__tests__/error-redaction.test.ts` is the standing check on that shape).
