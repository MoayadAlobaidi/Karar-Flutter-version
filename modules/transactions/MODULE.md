...
_None._ Every operation here is owner self-service: a principal reads and writes their OWN transactions and no other principal's. What denies is the principal resolved exclusively from the session's server-side tenant binding, `FinancialAccountAccessPort` (`application/ports/financial-account-access.ts`) resolving every anchor account as one the caller owns before a movement of money may be recorded, and RLS bound per transaction. `USER` holds nothing in the permission catalogue and that is the design, not an omission (`modules/authorization/domain/catalogue.ts`: "Own-data authority comes from identity + RLS, never from an RBAC grant"). A permission granted to `USER` and to no other role would therefore be a check that its only possible holder always passes — ceremony rather than a boundary. access-control.md §2 records the two authorisation models and which one governs here.

**Permissions deliberately absent:** No staff endpoint returns one customer's transactions, and none may be added. **No `?userId=` parameter is accepted anywhere.**
...
|---|---|---|
| `TransactionRetentionDecisionPort` | how long a transaction record may be kept: `DECIDED` · `PENDING_LEGAL_REVIEW` · `UNAVAILABLE` | `resolveTransactionRetentionDecisionPort`: the PolicyPack retention slot for the subject's jurisdiction, or, in a **local environment only**, `LocalSyntheticRetentionDecisionProvider`. Every other environment with no approved provider gets a typed throw |
...
| `DedupFingerprintPort` | the keyed, per-subject, versioned content identity of a movement — definition `dedup/hmac-sha256/calendar-day/v3`, stated in full below | `resolveDedupFingerprintPort`. LOCAL/TEST: `LocalKeyedDedupFingerprintProvider`. Production: a key-management-backed adapter (ADR-0017), and a typed throw until one is wired |
| `HsfFieldEncryptionPort` | per-field encryption of merchant, description and note | `resolveHsfFieldEncryptionPort`. LOCAL/TEST: `LocalAesGcmFieldEncryptionProvider`. Production: as above |
...
**Manual entry and CSV import are intended to be first-class rather than stopgaps** (challenge C9) — and today **both are running paths.** `CreateManualTransaction` is constructed in `apps/api/src/composition/phase5-modules.ts` and reached by `POST /financial/transactions`. The CSV pipeline exists in `modules/statement-imports` — parser, staging tables, import state machine and a reviewed commit that writes through `ImportedRecordCommitPort` above — and is reached by the seven mounted `/financial/statement-imports` operations. What no person has reached is either path *from a device*: no build has run on one. The normalisation rules below are exercised by the pipeline, not merely specified.
...
**Explicit ingestion limits are declared centrally and ENFORCED.** `packages/platform/src/ingestion/limits.ts` states them — bytes, rows, wall-clock, memory, batch — with no optional members and no way to express "unlimited", and its validator refuses non-finite, zero, negative and non-integer bounds. The rule they encode is reject rather than degrade (legacy FILES-2, HIGH). Architecture test 24 (resource limits declared) is **ACTIVE and passing**: it discovers real mounted ingestion paths from the tree and fails both when a mounted path declares no central policy and when a central policy names a path that no longer exists. This paragraph previously said the limits were "enforced by nothing", that "there is no ingestion path to apply them to", and that test 24 "stays phase-deferred" — all three became false when the CSV routes were mounted and `currentPhase` moved to 5, in the commit that activated the test.
