 ...
 * unbounded list.
 *
 * `expectEveryVisibleInstrument` asserts `hasMore` is false before handing
 * the rows back. Without it a probe would silently weaken the day a fixture
 * seeds more instruments than the limit: "this principal sees nothing of that
 * one" would start passing because the page ran out rather than because the
 * rows did.
 */
export const EVERY_INSTRUMENT_PAGE = {
  instrumentType: null,
  status: null,
  spendable: null,
  offset: 0,
...
export async function expectEveryVisibleInstrument(
  repository: PaymentInstrumentRepository,
  actor: InstrumentsPrincipal,
): Promise<readonly PaymentInstrument[]> {
  const page = await repository.pageOwn(actor, { accountRef: null, ...EVERY_INSTRUMENT_PAGE });
