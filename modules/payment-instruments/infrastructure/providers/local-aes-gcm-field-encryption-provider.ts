 ...
 *   also what makes the ciphertext non-deterministic, which matters more here
 *   than almost anywhere — a deterministic ciphertext over a four-digit card
 *   tail is a confirmation oracle over ten thousand values;
...
/**
 * The domain-separation label. Distinct from `karar/transactions/hsf/v1`,
 * `karar/financial-accounts/hsf/v1` and `karar/financial-connections/hsf/v1`
 * on purpose: even if the modules were ever wired to one key, a ciphertext
 * could not be replayed from one module's table into another's — and an
 * account mask and an instrument mask are close enough in shape that a replay
 * between them would decrypt to something plausible.
 */
const ASSOCIATED_DATA_LABEL = 'karar/payment-instruments/hsf/v1';
 ...
 * two different subjects whose cards sit in the same table.
 */
...
export class LocalAesGcmFieldEncryptionProvider implements HsfFieldEncryptionPort {
  ...
  constructor(options: {
    ...
    this.#keyVersion = options.keyVersion ?? 'karar-ref:key-version:local-payment-instruments-hsf@v1';
  }
 ...
 * encryption. The failure is at construction, before a single card mask has
 * been handled, because the alternative failure mode is a deployment that
 * quietly writes plaintext-equivalent rows and discovers it during an
 * incident.
 */
  ...
  if (options.env !== 'local') {
    throw new HsfFieldEncryptionError(
      ...
      `no approved HSF field-encryption provider is wired for KARAR_ENV='${options.env}', and ` +
        'there is no fallback: payment_instruments is HIGHLY_SENSITIVE_FINANCIAL and its mask ' +
        '— a fragment of a card number — exists only as ciphertext. Wire the environment’s ' +
