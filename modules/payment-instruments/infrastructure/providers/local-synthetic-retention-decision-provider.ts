 ...
 *
 * A synthetic answer that lets local development and the test suite record
 * payment instruments, labelled at every point where a human or a grep might
 * see it as having **no legal effect whatsoever**. The values are
 ...
 * for the same reason and with the same answer — the payment instruments this
 ...
 *
 * Dedicated `INSTRUMENT_SYNTHETIC_*` constants in the fixture package would be
  ...
  constructor(env: string) {
    super(
      `LocalSyntheticRetentionDecisionProvider is local-development-only and refuses to exist ` +
        ...
        `that used it would be recording real subjects' cards, and the encrypted fragments of ` +
        `card numbers attached to them, under a decision nobody took. Wire a provider that reads an ` +
...
export class LocalSyntheticRetentionDecisionProvider
  implements PaymentInstrumentRetentionDecisionPort
{
  readonly #env: string;
...
export function resolveRetentionDecisionPort(options: {
  readonly env: string;
  readonly approvedProvider?: PaymentInstrumentRetentionDecisionPort | null;
}): PaymentInstrumentRetentionDecisionPort {
