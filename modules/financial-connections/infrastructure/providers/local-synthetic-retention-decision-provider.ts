 ...
 * for the same reason and with the same answer — the connections and source
 * links this module owns: a subject's financial records, held under a legal
 * question nobody has answered. Reusing the account constants states that
 ...
 *
 * Dedicated `CONNECTION_SYNTHETIC_*` constants in the fixture package would
...
import type {
  FinancialConnectionRetentionDecisionPort,
  ...
  constructor(env: string) {
    super(
      `LocalSyntheticRetentionDecisionProvider is local-development-only and refuses to exist ` +
        ...
        `that used it would be recording real subjects' data routes and the encrypted external ` +
        `identifiers behind them under a decision nobody took. Wire a provider that reads an ` +
...
export class LocalSyntheticRetentionDecisionProvider
  implements FinancialConnectionRetentionDecisionPort
{
  readonly #env: string;
...
export function resolveRetentionDecisionPort(options: {
  readonly env: string;
  readonly approvedProvider?: FinancialConnectionRetentionDecisionPort | null;
}): FinancialConnectionRetentionDecisionPort {
