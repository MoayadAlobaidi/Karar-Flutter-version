/**
 * Expected failure shapes of this module's use cases (backend.md §9). Every
 * kind is machine-readable for RFC 7807 mapping.
 *
 * **Denial kinds deliberately avoid oracles.** A transaction id that does not
 * exist, one belonging to another user in the same tenant, and one belonging
 * to another tenant all answer `transaction_not_found` — identically, with the
 * same message; the same holds for a match id. Anything else turns a guessed
 * identifier into a membership test. RLS produces this outcome structurally
 * (the row is simply not visible), and these types keep the application layer
 * from reintroducing the distinction on the way out.
 *
 * **No message in this file carries an amount.** The refusals are about a
 * person's money and every one of them describes the RULE rather than the
 * figures — an error string that quotes two amounts puts financial values into
 * log lines and RFC 7807 bodies.
 *
 * **No message interpolates driver text.** `String(error)` and
 * `error.message` never appear in a caller-visible sentence: a driver message
 * can carry a connection string with credentials, the failing SQL, or a
 * fragment of a person's record. The original throw rides along
 * NON-ENUMERABLE for the one boundary allowed to log it, exactly as
 * `modules/financial-accounts/application/errors.ts` does — a field that must
 * not be serialized is safer as a field that cannot be.
 */

import type { TransferMatchRuleViolation } from '../domain/errors.js';
import type {
  FinancialRetentionDecision,
  RetentionGovernedDataset,
} from './ports/transfer-match-retention-decision.js';
import type { MissingPrincipalContext } from './principal.js';

/** No transfer match with that id is visible to the acting principal. */
export interface MatchNotFound {
  readonly kind: 'match_not_found';
  readonly message: string;
}

/**
 * One of the two transactions is not visible to the acting principal —
 * absent, another user's, another tenant's, or never minted,
 * indistinguishably.
 *
 * `side` says WHICH of the two, and that is safe: a caller supplied both ids
 * and learns only which of their own inputs failed, never anything about a
 * row belonging to anybody else.
 */
export interface TransactionNotFound {
  readonly kind: 'transaction_not_found';
  readonly side: 'FIRST' | 'SECOND';
  readonly message: string;
}

/**
 * One of the two transactions already belongs to a live match.
 *
 * Carries the colliding match id, because that match is the SUBJECT'S OWN row
 * — telling them which one is the entire point, and a caller who could not
 * already see it could not have reached this arm.
 */
export interface TransactionAlreadyMatched {
  readonly kind: 'transaction_already_matched';
  readonly conflictingMatchId: string;
  readonly collision: 'SAME_SIDE' | 'CROSSED_SIDES';
  readonly message: string;
}

/** A domain rule refused the request; `violation` says which. */
export interface RuleViolated {
  readonly kind: 'rule_violated';
  readonly violation: TransferMatchRuleViolation;
  readonly message: string;
}

/**
 * The match changed since the caller read it. The caller re-reads and decides;
 * this module never silently overwrites a concurrent edit, because the losing
 * edit is usually the one a person made on their other device — and here the
 * losing edit is a DECISION about their own money.
 */
export interface VersionConflict {
  readonly kind: 'version_conflict';
  readonly expectedVersion: number;
  readonly message: string;
}

/**
 * The store failed. Never carries store internals outward.
 *
 * `operation` is this module's own vocabulary ('suggest transfer match'),
 * never the store's. `cause` holds the original throw for the ONE place
 * allowed to log it — the boundary that turns this into a response — and is
 * defined NON-ENUMERABLE so `JSON.stringify`, object spread, structured
 * logging and an RFC 7807 body all drop it without anyone remembering to.
 */
export interface StoreFailure {
  readonly kind: 'store_failure';
  readonly message: string;
  readonly operation: string;
  /** Non-enumerable; present for the boundary logger, invisible to serialization. */
  readonly cause?: unknown;
}

/**
 * Whether a transaction is visible could not be established, so the match was
 * refused. Fail closed: "we could not check, so probably fine" is how a match
 * gets written across two subjects.
 */
export interface TransactionAccessUnavailable {
  readonly kind: 'transaction_access_unavailable';
  readonly message: string;
  /** Non-enumerable; present for the boundary logger, invisible to serialization. */
  readonly cause?: unknown;
}

/**
 * Durable creation was refused because the retention decision governing the
 * dataset is not resolved. Carries the decision itself, not a rephrasing:
 * "legal has not ruled" and "we could not ask" have different remedies.
 */
export interface RetentionUnresolved {
  readonly kind: 'retention_unresolved';
  readonly dataset: RetentionGovernedDataset;
  readonly decision: FinancialRetentionDecision;
  readonly message: string;
}

export type SuggestTransferMatchError =
  | MissingPrincipalContext
  | RetentionUnresolved
  | TransactionNotFound
  | TransactionAccessUnavailable
  | TransactionAlreadyMatched
  | RuleViolated
  | StoreFailure;

export type ConfirmTransferMatchError =
  | MissingPrincipalContext
  | MatchNotFound
  | RuleViolated
  | VersionConflict
  | StoreFailure;

export type RejectTransferMatchError =
  | MissingPrincipalContext
  | MatchNotFound
  | RuleViolated
  | VersionConflict
  | StoreFailure;

export type ListOwnTransferMatchesError = MissingPrincipalContext | StoreFailure;

export type EraseTransferMatchesError = MissingPrincipalContext | StoreFailure;

/** The one message every match not-found arm uses. */
export const MATCH_NOT_FOUND: MatchNotFound = Object.freeze({
  kind: 'match_not_found' as const,
  message:
    'no such transfer match — a guessed identifier, a match belonging to another user inside ' +
    'this tenant, and one belonging to another tenant are all answered identically on purpose, ' +
    'so a caller learns nothing from the difference',
});

/** The one message every transaction not-found arm uses, per side. */
export function transactionNotFound(side: 'FIRST' | 'SECOND'): TransactionNotFound {
  return {
    kind: 'transaction_not_found',
    side,
    message:
      'no such transaction — a guessed identifier, a transaction belonging to another user ' +
      'inside this tenant, and one belonging to another tenant are all answered identically on ' +
      'purpose. A match is not created across two subjects, and the refusal reveals nothing ' +
      'about whether the transaction exists for anybody else',
  };
}

/**
 * Wrap an unexpected store throw without leaking its internals to a client.
 *
 * A driver message is not ours to show: it can carry a connection string with
 * credentials, the SQL that failed, a table name, a host and port, or a
 * fragment of the row that failed. It is also unstable, so a client that keyed
 * on it would break on a driver upgrade.
 */
export function storeFailure(operation: string, error: unknown): StoreFailure {
  const failure = {
    kind: 'store_failure' as const,
    operation,
    message:
      `${operation} could not be completed because the store did not answer. The reason is ` +
      'deliberately not described here: it comes from the database driver, and driver text can ' +
      'carry credentials, SQL, or a fragment of the record itself. It is logged once at the ' +
      'boundary, against this request',
  };
  // Non-enumerable: invisible to JSON.stringify, spread, and any serializer,
  // reachable only by code that names it.
  Object.defineProperty(failure, 'cause', { value: error, enumerable: false, writable: false });
  return failure as StoreFailure;
}

/** The refusal when transaction visibility could not be established. */
export function transactionAccessUnavailable(error: unknown): TransactionAccessUnavailable {
  const failure = {
    kind: 'transaction_access_unavailable' as const,
    message:
      'whether these transactions are ones the caller owns could not be established, so no ' +
      'match was suggested. The rule fails closed: a match written against an unverified ' +
      'transaction reference could pair one person’s movement with another’s, and nothing ' +
      'downstream would ever question it. Why the answer went missing is logged once at the ' +
      'boundary',
  };
  Object.defineProperty(failure, 'cause', { value: error, enumerable: false, writable: false });
  return failure as TransactionAccessUnavailable;
}

/**
 * The one refusal the retention gate produces, so the wording cannot drift
 * and so the reason is always the decision's own.
 */
export function retentionUnresolved(
  dataset: RetentionGovernedDataset,
  decision: FinancialRetentionDecision,
): RetentionUnresolved {
  const because =
    decision.state === 'DECIDED'
      ? 'the decision claims DECIDED but carries no basis or no approval reference, and absence ' +
        'of evidence means not approved'
      : decision.reason;
  return {
    kind: 'retention_unresolved',
    dataset,
    decision,
    message:
      `refusing to create a durable record in ${dataset}: the retention decision governing it is ` +
      `${decision.state} — ${because}. This module's data-lifecycle declaration says non-local ` +
      'durable financial creation fails closed until a reviewed decision exists, and this is ' +
      'that gate; the remedy is a legal decision recorded in an approved policy pack, never a ' +
      'duration chosen in code',
  };
}
