/**
 * `TransferMatchRetentionDecisionPort` — the retention question, asked
 * before anything durable is written, declared INWARD.
 *
 * ## The claim this port exists to make true
 *
 * Migration 0099's header and this module's MODULE.md both say:
 *
 ...
 *
 * The table stores no amount and no narrative — only identifiers, a state and
 * a decision instant. It is still a `HIGHLY_SENSITIVE_FINANCIAL`
 * subject-owned record: it says which of a person's movements were transfers
 * between their own accounts, which is a map of how their money circulates
 * and which of their accounts feed which. "It is only a relationship" is
 * exactly the argument that would leave a subject's financial structure
 * ungoverned.
 *
 * ## The vocabulary
 *
 ...
 * `modules/financial-accounts`, `modules/financial-connections` and
 * `modules/payment-instruments`. A
 ...
 * record of which of a person's movements were transfers between their own
 * accounts — and which of those they confirmed — is outside retention law. That is a defect in the provider, and
 ...
 */
export const RETENTION_GOVERNED_DATASETS = ['transfer_matches'] as const;
...
export interface TransferMatchRetentionDecisionPort {
  /**
