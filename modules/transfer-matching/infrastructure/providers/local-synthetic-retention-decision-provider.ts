 ...
 *
 * A synthetic answer that lets local development and the test suite suggest
 * transfer matches, labelled at every point where a human or a grep might see
 * it as having **no legal effect whatsoever**. The values are
 ...
 * decision, and it governs the accounts, the balances, the transactions and —
 * for the same reason and with the same answer — the transfer matches this
 * module owns: a subject's financial records, held under a legal question
 * nobody has answered. Reusing the account constants states that
 * relationship rather than manufacturing a second synthetic answer that could
 * drift from the first, and the values are self-describing, so a value quoted
 * out of context still announces what it is.
 *
 * Dedicated `TRANSFER_MATCH_SYNTHETIC_*` constants in the fixture package would
 * be tidier and are recorded in this module's MODULE.md as a change the
  ...
  constructor(env: string) {
    super(
      `LocalSyntheticRetentionDecisionProvider is local-development-only and refuses to exist ` +
        ...
        `that used it would be recording which of real subjects' movements were transfers ` +
        `between their own accounts, under a decision nobody took. Wire a provider that reads an ` +
        `approved policy pack, or leave durable creation refusing`,
    );
  }
}

export class LocalSyntheticRetentionDecisionProvider
  implements TransferMatchRetentionDecisionPort
{
  readonly #env: string;
...
export function resolveRetentionDecisionPort(options: {
  readonly env: string;
  readonly approvedProvider?: TransferMatchRetentionDecisionPort | null;
}): TransferMatchRetentionDecisionPort {
