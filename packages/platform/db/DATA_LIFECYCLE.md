...
| `public.subject_policy_selections`             | `SUBJECT_OWNED`      | record which pack-permitted profile option the subject elected per capability, with pinned jurisdiction/pack/profile versions, so historical resolutions replay under the conventions that produced them (migration 0083) | `CONFIDENTIAL` | from PolicyPack per jurisdiction (Phase 3.5+); interim policy-configuration placeholder: life of the account plus 13 months after supersession/withdrawal, never a code constant                                                                                                            | included — the subject's export contains their own election history with its pinned versions                                                       | `RETAIN_WITH_BASIS`      |
| `public.capability_availability`               | `NON_PERSONAL`       | audited, restrict-only capability exposure state per (environment, jurisdiction?, capability); a missing row means DISABLED (migration 0076)                                                                              | `INTERNAL`     | current configuration lives with the platform; PolicyPack owns any bound (Phase 3.5+), never a code constant                                                                                                                                                                                | n/a — no subject owns configuration                                                                                                                | `NON_PERSONAL_BY_DESIGN` |
| `public.capability_availability_history`       | `NON_PERSONAL`       | append-only ledger of every availability state that ever held — the referent of the resolver's TOCTOU provenance pins (migration 0076)                                                                                    | `INTERNAL`     | configuration history explains every past resolution; PolicyPack owns any bound (Phase 3.5+)                                                                                                                                                                                                | n/a                                                                                                                                                | `RETAIN_WITH_BASIS`      |
| `public.tenant_capability_entitlements`        | `NON_PERSONAL`       | deny-by-default per-tenant capability entitlement with opaque source seam, effective window, and accountability (migration 0077)                                                                                          | `INTERNAL`     | life of the tenant relationship plus the PolicyPack's post-termination period (Phase 3.5+), never a code constant                                                                                                                                                                           | n/a — no subject owns a tenant entitlement                                                                                                         | `RETAIN_WITH_BASIS`      |
| `public.tenant_capability_entitlement_history` | `NON_PERSONAL`       | append-only ledger of every entitlement state that ever held — the referent of the resolver's TOCTOU entitlement pins (migration 0077)                                                                                    | `INTERNAL`     | entitlement history explains every past resolution; PolicyPack owns any bound (Phase 3.5+)                                                                                                                                                                                                  | n/a                                                                                                                                                | `RETAIN_WITH_BASIS`      |
| `public.countries`                             | `NON_PERSONAL`       | geographic reference — ISO 3166-1 codes with display keys and default-currency formatting references; data, never policy (migration 0070)                                                                                 | `PUBLIC`       | life of the platform reference set; PolicyPack owns any bound (Phase 3.5), never a code constant                                                                                                                                                                                            | n/a — no subject owns a reference row                                                                                                              | `NON_PERSONAL_BY_DESIGN` |
| `public.jurisdictions`                         | `NON_PERSONAL`       | the legal-regime register runtime rows key on — the policy key, kept distinct from country so historical records never need re-derivation (migration 0071)                                                                | `INTERNAL`     | life of the platform register; PolicyPack owns any bound (Phase 3.5), never a code constant                                                                                                                                                                                                 | n/a — no subject owns a register row                                                                                                               | `NON_PERSONAL_BY_DESIGN` |
| `public.user_jurisdiction_assignments`         | `SUBJECT_OWNED`      | resolve which legal regime governs this user now and at any past instant, with source and verification provenance (migration 0072)                                                                                        | `CONFIDENTIAL` | from PolicyPack per jurisdiction (Phase 3.5); assignment history explains which regime governed which period of the subject's records, never a code constant                                                                                                                                | included — a subject's export names their regime bindings and since when                                                                           | `RETAIN_WITH_BASIS`      |
| `public.tenant_jurisdiction_assignments`       | `NON_PERSONAL`       | resolve which legal regime a tenant organisation operates under, now and at any past instant, with source and verification provenance (migration 0073)                                                                    | `INTERNAL`     | from PolicyPack per jurisdiction (Phase 3.5); regime history explains which rules governed which period of the tenant's operation                                                                                                                                                           | n/a — no subject owns a tenant regime row                                                                                                          | `RETAIN_WITH_BASIS`      |
| `public.jurisdiction_settings`                 | `NON_PERSONAL`       | restrict-only operational narrowing of the code PolicyPack per jurisdiction — disable a capability now, suspend AI processing now (migration 0074)                                                                        | `INTERNAL`     | current operational state lives with the platform; PolicyPack owns any bound (Phase 3.5)                                                                                                                                                                                                    | n/a — no subject owns an operational settings row                                                                                                  | `NON_PERSONAL_BY_DESIGN` |
| `public.policy_pack_activations`               | `NON_PERSONAL`       | append-only record of which pack VERSION was operative per (jurisdiction, environment), from when, decided by whom and why; metadata only, never pack content (migration 0075)                                            | `INTERNAL`     | activation history explains every past policy resolution; PolicyPack owns any bound (Phase 3.5)                                                                                                                                                                                             | n/a — no subject owns an activation event                                                                                                          | `RETAIN_WITH_BASIS`      |
...
Subject-policy basis notes (migration 0083; canonical declaration in
[`modules/subject-policy/MODULE.md`](../../../modules/subject-policy/MODULE.md),
mirrored here because a platform migration created the table):

- `public.subject_policy_selections` — `RETAIN_WITH_BASIS`: the basis is
  provenance of processing already performed — a selection pins the
  jurisdiction, PolicyPack version, and profile version that produced every
  historical resolution under the subject's elected conventions
  (jurisdiction-policy.md §7 rule 3), so the election record must outlive
  the election. Rows are immutable by trigger (re-election inserts, never
  edits); `user_id` and `recorded_by` are opaque identity references that
  resolve to nothing once the subject is erased. `CONFIDENTIAL` is the
  floor: an elected methodology can reveal religious affiliation (§7 rule
  5), so the table is RLS ENABLEd and FORCEd on both principal GUCs with no
  allow-list entry, `karar_app` holds no DELETE, and export includes the
  subject's own rows only. Option CONTENT never enters the table — content
  is capability-owned; the row stores references and version pins plus an
  optional content-snapshot HASH.

<!-- Phase 3.5 capability workstream: rows for migrations 0076-0077 live in the
     main table above (test 25 parses this file as ONE six-field table, so a
     second header/separator would be validated as data rows); canonical
     declarations in modules/capability/MODULE.md; mirrored here because
     platform migrations create the tables. -->

Capability workstream basis notes:

- `public.capability_availability` — `NON_PERSONAL_BY_DESIGN`: rows hold
  environment, jurisdiction and capability references, a closed state, an
  operator reference string, and a reason; nothing re-identifies a person.
  Deleting the current row is forbidden anyway (withdrawal is state
  `DISABLED`, by trigger), which is a restrict-only safety property, not a
  personal-data one.
- `public.capability_availability_history` and
  `public.tenant_capability_entitlement_history` — `RETAIN_WITH_BASIS`: the
  basis is exposure accountability — why the platform ever answered
  AVAILABLE (or refused to) for a capability at time t, on whose decision,
  at which pinned version. The ledgers are written only by SECURITY DEFINER
  triggers and are what the resolver's §44 provenance pins refer to; rows
  name operator references, never subjects.
- `public.tenant_capability_entitlements` — `RETAIN_WITH_BASIS`: a fact about
  a tenant organisation's contracted capability set (tenants are
  `NON_PERSONAL`, see `public.tenants`); the basis is access accountability
  across the tenant relationship. `source_ref` is an opaque reference — no
  subscription, plan, or pricing data exists in this phase.

Jurisdiction workstream basis notes (migrations 0070-0075; canonical
declarations in [`modules/jurisdiction/MODULE.md`](../../../modules/jurisdiction/MODULE.md),
mirrored here because platform migrations create the tables):

- `public.countries` and `public.jurisdictions` — `NON_PERSONAL_BY_DESIGN`,
  with the demonstration ADR-0026 requires: rows hold ISO codes,
  localization keys, currency codes, regime type/lifecycle labels, and
  provenance prose about the platform's own declarations, seeded by
  migration only (`karar_app` is SELECT-only on both). No column references
  a person and no linkage — restorable or otherwise — to any subject
  exists, so re-identification is impossible by construction rather than by
  processing. A country row carries no business rule; the policy key is the
  jurisdiction, and keeping the two apart is what stops historical records
  from ever needing re-derivation (jurisdiction-policy.md §1).
- `public.user_jurisdiction_assignments` — `RETAIN_WITH_BASIS`: where a
  person falls under law is personal data (`CONFIDENTIAL`), and the basis
  for retaining it is that every jurisdiction-pinned record must stay
  explainable — which regime governed the subject when the record was
  made, on what source, verified or not. `user_id` and `tenant_id` are
  opaque cross-module references with no foreign key, so the accountability
  fact survives erasure of the subject's identity and then resolves to
  nothing. Rows are end-only history (guard trigger); `karar_app` holds no
  DELETE.
- `public.tenant_jurisdiction_assignments` — `RETAIN_WITH_BASIS`: the same
  accountability basis for an organisation rather than a person (tenants are
  `NON_PERSONAL`, see `public.tenants`); the regime history explains which
  rules governed which period of the tenant's operation.
- `public.jurisdiction_settings` — `NON_PERSONAL_BY_DESIGN`: rows hold a
  jurisdiction code, capability id strings, a suspension flag, a version, an
  operator reference, and a reason; `updated_by` names an operator acting in
  an official capacity, and no subject data exists to re-identify. The row
  can only ever RESTRICT what the code PolicyPack permits — deleting one
  removes a restriction, never a grant.
- `public.policy_pack_activations` — `RETAIN_WITH_BASIS`: the basis is
  policy accountability — every past resolution must be explainable (which
  pack version was operative, in which environment, from when, on whose
  decision, for what reason). Append-only by both mechanisms (SELECT+INSERT
  grants and the immutability trigger); the ledger stores version metadata
  only, never pack content, because packs are reviewed code.
