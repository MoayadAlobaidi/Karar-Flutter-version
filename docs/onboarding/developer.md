...
**Two caveats, because the tree is behind that rule in two different ways.** The route-level half is aspirational everywhere: `requirePermission(...)` is declared in `modules/authorization/presentation/http/` and has **no production call site** — the Phase 3 and 3.5 modules do their enforcement in the use-case half, calling `authorize(...)` through each module's own `PolicyService` port. And the **Phase 5 financial surface does neither**: its six modules declare twelve permissions, none is seeded by a migration, and no route or use case consults one. That surface denies through the session's server-side tenant binding, the application-layer account access ports and RLS instead, which is a real boundary but a different one. Follow the rule above for anything you add; do not read the financial controllers as the pattern.
...
**The drift gate binds contract to client. A separate suite binds server to contract, and you are expected to extend it with your change.** `tests/conformance` drives the composed application over real HTTP against live PostgreSQL and Redis, and validates the status, the `Content-Type` and the returned bytes against the OpenAPI document. It covers 148 of the merged contract's 273 declared operation/status pairs today — 82 of the 128 non-financial ones, plus 66 of the 145 the Phase 5 `/financial/*` fragments declare, in a second file with its own ledger so that one failure cannot hide the rest — and two ledgers assert **empty**: no problem document may leave under `application/json` when the contract declares `application/problem+json` (throw it and let the error boundary set the media type — do not write to the reply object), and **no operation may describe its response in prose without a schema**, because a schema-less operation is one neither the generated client nor this suite can check.
