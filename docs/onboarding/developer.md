...
| Architecture | 26 CI-blocking tests plus a canary-purity check; the current run is 24 passed, 0 failed, 4 deferred by activation phase, with a self-test on the same invocation |
...
Phase 5 — the financial data platform — is IN PROGRESS; the last completed phase is 4 (the Flutter and mobile security foundation), merged to `main` on 18 August 2026. **Phase 6 has not started.** Phase 5 has built its **data foundation only**: schema, domain, ports and repositories across five modules — `financial-accounts`, `transactions`, `financial-connections`, `payment-instruments`, `transfer-matching` — behind migrations 0087-0099, and **nothing reaches them**: no route, no client method, no screen, nothing deployed, and no ingestion path of any kind.

**The architecture-test registry's `currentPhase` is still 4, deliberately.** Advancing it activates architecture test 24 (resource limits declared), whose own activation criterion is that a first ingestion path exists — and none does, so the test would scan nothing and pass vacuously. The lock runs both ways: a supplementary check refuses a pre-phase-5 tree that mounts an ingestion path at all. `currentPhase` moves to 5 in the same change that lands the first mounted ingestion path, and not before.
...
Consumer product capabilities. Phase 3.5 delivered the policy and capability machinery, Phase 4 built a client that consumes it, and Phase 5 is building the financial data foundation — but **no capability is implemented or reachable**: every registry entry is `NOT_IMPLEMENTED` and deployed nowhere, and the client's navigable-capability set is correspondingly empty. **The Phase 5 financial tables are no exception** — they exist in the schema and in five modules, and no route, use-case wiring or client method reaches them; `TRANSACTIONS` is not available in any PolicyPack.

Specifically **not built** inside Phase 5's own scope, so nobody infers it from the presence of schema: manual transaction entry as a running path (the use case exists and nothing calls it), **CSV statement ingestion** (no parser, no import state machine, nothing mounted), the categorization pipeline, every OpenAPI operation, and every client surface. **No provider is connected** — no issuer named in the catalogue exposes an interface to Karar, no credential of any kind is stored anywhere, and only the `MANUAL` and `USER_FILE_UPLOAD` rails may exist, enforced by a database CHECK. Nothing may display "Connected" for data a person typed or uploaded. **The financial-data retention decision is legal and has not been taken**, so this data cannot reach any deployed environment.
