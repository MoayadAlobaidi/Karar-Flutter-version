...
- A fabricated account, connection, or sync status. This was the legacy's worst surface, and Phase 5 has made the temptation concrete rather than hypothetical: the server now holds accounts, connections, payment instruments and imported statements, **and 27 `/financial/*` operations answer for them.** The generated client carries the matching 27 methods, because it is generated from the whole contract. **Seven feature folders now call those methods, and every route is gated inside its builder on the platform's own capability answer** — but a capability becomes navigable by being implemented *and* deployed *and* deliberately added to the set, and nothing is deployed, so `navigableCapabilityIds` is still empty. The screens were written, and the word "Connected" is still forbidden, because only the `MANUAL` and `USER_FILE_UPLOAD` rails may exist, no credential is stored anywhere, and no issuer exposes an interface to Karar. The server cannot express the claim; the client must not invent it in a label.
