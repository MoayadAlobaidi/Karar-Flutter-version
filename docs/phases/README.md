...
| [`phase-05.md`](phase-05.md) | 5 — Financial data platform: issuers and markets, accounts and wallets, transactions, connections and source links, payment instruments, transfer matching; manual and CSV ingestion; erasure strategies enforced. **IN PROGRESS — data foundation only, across five modules and migrations 0087-0099, and reachable by nothing. Manual entry, CSV ingestion, categorization as a running path, every API operation and every client surface are not built; retention is unresolved; `currentPhase` is still 4** |

The Phase 5 row is the one place where the index and the roadmap could most easily drift apart, so it says the same three things both documents say: what exists, what does not, and why the phase marker has not moved. **Phase 6 has not started**, and nothing in it is pre-activated by the presence of Phase 5 schema.
