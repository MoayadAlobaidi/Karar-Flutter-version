...
**The Flutter numbers above were inherited from Phase 4 and were badly stale**, on the reasoning that "this change touches no Dart or platform code" — which stopped being true when the client surface landed. Re-measured on a stable tree at the checkpoint: **Flutter 2028 passed / 1 skipped**, goldens **4**, localization **38**, mobile security **149 passed / 1 skipped**. `flutter analyze` reports no issues, and `dart run tool/generate_api_client.dart --check` reports the client in sync (62 operations, 203 schemas).

**The workspace suite is 2973 passed / 12 skipped / 6 failed, and the six are recorded as failures rather than re-run until green.** All six are five-second timeouts in the three suites that provision and drop whole databases — `packages/platform/src/db/contract.test.ts` (four), `modules/audit` and `modules/identity` (one each). Run alone on the same machine they are **37 passed in 13 seconds**. That is a resource observation and not a conformance result, and it is the same shape as the login timeout recorded above: the assertions never ran rather than running and disagreeing. It is reported here anyway, because a report that quietly reruns until green is not evidence.

**Architecture and documentation figures, re-measured at the same checkpoint** (the numbers this section previously carried were taken at `ef1d155` and had drifted): `pnpm arch:test` prints **27 passed, 0 failed, 3 skipped**, self-test **70 cases**, and test 24 scans **50** — 48 surface files plus the two central policies, which it did not read at all until the offset defect recorded under *Phase activation* was fixed. There are **three** supplementary checks, not two, and all three now count on both arms, so the reconciliation paragraph that used to explain a one-off discrepancy between `25 passed` and the registry-derived figure no longer describes anything and has been removed. `pnpm docs:check` prints **13/13**. `pnpm typecheck`, `pnpm build` and `pnpm lint` all exit **0**.
## Build results
