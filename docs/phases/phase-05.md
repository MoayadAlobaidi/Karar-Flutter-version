...
| Suite | Result |
|---|---|
| Workspace (`pnpm test`, ordinary parallel) | 1896 passed / 12 skipped / **0 failed** (1908 total) |
...
**The ordinary parallel invocation is now reliable, and was not before.** It failed intermittently for two separate reasons that have both been closed: F4 (a real defect — a concurrent dedup loser arrived untyped) and connection exhaustion on a 12-core machine, which made suites SKIP rather than fail so the run stayed green while it had quietly stopped verifying. The worker count now derives from the connection budget, and `KARAR_INTEGRATION=1` makes an unreachable database a failure. Evidence: **ten consecutive `pnpm test` runs, 10/10 passed, identical 12 skips each time, zero orphan scratch databases.**
