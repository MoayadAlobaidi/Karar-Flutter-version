...
**Every figure below was re-derived at the CLOSE-OUT head during the Phase 5 closeout verification**, on a settled tree with nothing else running, against a PostgreSQL 17.11 instance created from an empty volume. The two-heads arrangement this paragraph used to describe — tree-derived checks at one commit and assertion counts at another — is gone: everything here comes from one head, and the head is the one this phase's pull request carries.
...
|---|---|---|
| Architecture (`pnpm arch:test`) | **27 passed, 0 failed, 3 deferred to phase 13, 1 NOT APPLICABLE**; registry errors 0; self-test PASS over **86** cases. Three supplementary checks pass; the fourth — the retired pre-phase-5 guard — reports `N/A` with its reason and is **excluded from the pass count** | close-out |
| Documentation (`pnpm docs:check`) | **15/15**, self-test ok over **31** cases across two fixtures, 293 markdown files scanned | close-out |
| Prisma mapping (`node scripts/db/prisma-mapping-check.mjs`) | **61 mapped tables match the live database** | checkpoint |
| Workspace (`pnpm test`) | **3146 passed / 12 skipped / 0 failed (3158 total)** across 222 files (221 passed, 1 skipped), on PostgreSQL 17.11, **identical under both server default timezones** | close-out |
...
| Flutter (`flutter test`) | **Three figures, because one cannot carry it honestly.** With build artifacts present and goldens excluded as CI runs them: **2069 passed / 1 skipped**. The four golden baselines pass separately, giving **2073 passed / 1 skipped** for the whole suite. In a tree with **no** artifacts — the shape CI's `mobile` lane runs, which builds none — the same command gives **2051 passed / 19 skipped**; 2051 + 19 and 2069 + 1 are the same 2070 tests, and the 18 that move are artifact-gated assertions that **fail rather than skip** on the `mobile-android` and `mobile-ios` lanes. `flutter analyze` reports no issues | close-out |
...
| N5 | HIGH (verification finding, not a code defect introduced by this phase) | **The mobile client, executed on a runtime for the first time, does not reach a screen.** See below | Not fixed — reproduced, recorded, and owned | KAR-RSK-042, CI-020 | — |
...
The startup coordinator renders that indicator for exactly three states — `configLoading`, `sessionRestoring`, `bootstrapLoading` — and since no bootstrap request was ever issued, the sequence is suspended in one of the two **secure-storage reads that precede the network**: `AppLockGate.load()` or `SessionManager.restore()`. Every path involved is covered in the test suite through **fakes only**. The Dart VM service was reachable and `getVM` answered; `getStack` did not return.

**Two things this is NOT.** It is not a claim about a physical device — no Android or iOS hardware was attached, and a simulator is not a device. And it is not a Phase 5 regression: the startup sequence is Phase 4 code, unchanged by this phase.

**What it changes is the standing of every client claim.** Until this session, "no device execution" recorded an absence of proof. It now records a **demonstrated failure to start** on the one runtime anyone has tried, and the surface Phase 5 built has therefore never been reached by anything. It is KAR-RSK-042 with a treatment, CI-020 in the improvement log, and a Phase 6 entry criterion.
...
**There are no `TODO`, `FIXME`, `HACK` or `XXX` markers in this repository's own source.** Five matches exist across the whole tree: three in generated Prisma runtime typings, and two where `XXX` is a deliberately invalid ISO 4217 currency code in a test. **No application-layer class is unreferenced** — a sweep over every `modules/*/application/use-cases/` export found zero with no production reference outside its own file.

**Ten Phase 5 use cases are reachable from their module's public API and NOT from `apps/api`.** That is the real inventory of "built but not mounted", and each row below says why, who owns it, and what closes it. The distinction the table keeps is the one the owner asked for: a **later-phase item** is scoped work nobody has started, and an **unfinished Phase 5 item** is work this phase claimed. Only the last two rows are the second kind.
...
| **The client does not reach a screen** | `lib/app/lifecycle/startup_coordinator.dart` (suspended in `AppLockGate.load()` or `SessionManager.restore()`) | **Not deferred — found during this verification.** The first execution of the client on any runtime showed it stuck on the transient startup indicator, issuing no request | Engineering Owner · **Phase 6 entry** (KAR-RSK-042, CI-020) | Every claim about client behaviour rests on unit tests against fakes. The surface Phase 5 built has never been reached | **It blocks the claim, not the code.** Nothing in the backend depends on it, and the phase report never claimed the client ran | Reproduction on a second runtime, a fix, and an automated check that launches the artifact and asserts a terminal startup state |
| **Protected evidence store (KAR-RSK-011)** | — | **Not deferred — overdue.** Due at the Phase 2 gate, restated as "before Phase 4", recorded SLIPPED at Phase 4, and missed again here | Compliance Owner · **hard deadline before the Phase 6 gate report** (CI-016) | All compliance evidence sits in one vendor account under one credential with no independent custodian. This is why **no evidence row anywhere is `REVIEWED`** | **No for engineering; it is a compliance nonconformity with a written risk acceptance** | A store decision recorded with its access design and an approval |
...
1. **`KAR-RSK-042` has a reproduction on a second runtime and a fix, with an automated check that launches the built artifact and asserts a terminal startup state is reached.** Phase 6 builds a financial engine whose output the client renders. Starting it while the client cannot start is building on something unverified.
...
3. **`CI-011` discharges or opens a nonconformity of its own.** KAR-CTL-116's merge-blocking register sweep has now missed two targets.
