...
| Documentation (`pnpm docs:check`) | **16/16**, self-test ok over **52** cases across two fixtures, 293 markdown files scanned | close-out |
...
**Fixed twice, and the first fix was worse than the problem.** It waited for the stream's filter banner as evidence of attachment. The banner is printed when the predicate is PARSED, about twenty milliseconds in, so the "bounded twenty-second wait" terminated in under one second — shorter than the two-second sleep it replaced, on the exact failure it was written for. A fifth reviewer measured it. There is no signal on that channel meaning *subscribed*, so the wait is now unconditional and generous, and the archive scan is what makes the check reliable rather than a decoration on it. **The fallback was also picking the wrong announcement.** `log show` emits oldest-first and the fallback took the first match, which is the one most likely to belong to a PREVIOUS launch — so on a developer's own simulator the probe could query a dead isolate and report a tree the app under test never drew, and a release build on a device that had run a debug build minutes earlier would find that older announcement and proceed, defeating this check's own refusal to guess. The window is now the time since this run launched, and the newest match wins.
...
## Five review cycles, and the finding that outlived all of them

**This is the result of the closeout, stated before the detail: the engineering half did not converge.**

| Cycle | Head | Found | Where |
|---|---|---|---|
| 1 | `341c479` | 2 BLOCKING, 5 HIGH | the tree, and the corpus |
| 2 | `d10f0df` | 3 HIGH | inside the checks written to catch cycle 1 |
| 3 | `3cf1bb6` | 1 BLOCKING, 3 HIGH | the blocker inside that commit's headline act |
| 4 | `822a2f4` | 1 BLOCKING, 3 HIGH | the class the previous fix named and did not close |
| 5 | `83ddfb9` | 1 BLOCKING, 7 HIGH | **the same blocker as cycle 4, still live** |

**Cycle 5's blocker was cycle 4's blocker.** CI-020 appeared as CLOSED in five live places while the improvement log recorded it OPEN. Three were corrected; two survived, and they survived because a human reading the diff could not see them — they were not sentences. They were `KAR-RSK-042 CLOSED, CI-020 CLOSED` inside a table cell.

**And the rule built to catch that blocker was blind to every shape the blocker actually took.** The fifth reviewer recovered the literal bytes from the parent commit and ran the pattern against them: **0 of 5 matched.** It fired only on the sentence the fix had rewritten the blocker *into*, and on the sentence its own fixture seeded.

**That is the mechanism of the whole five-cycle failure, and it is worth stating plainly.** Every fixture in this repository was written from the *repaired* text. A self-test built that way proves the rule accepts a corrected corpus — never that it rejects the defect. Four cycles of rules each passed their own tests and each missed the thing they were built for.

**The fix is structural and it is applied.** The traceability cases are now seeded from the **pre-fix bytes**: the plural copula the blocker used, the verbless table-cell adjacency that survived two cycles, and a transition arrow that must not be mistaken for a status. Removing either new form fails its own case. The rule also learned that a status column is short and a paragraph in a table is prose, because the first attempt at the verbless form reported this report's own sentences describing the defect.

**Other findings from cycle 5, all fixed:** the CI register's status vocabulary was transcribed from the two values the table happened to hold rather than the four its schema defines, so a row marked `IN_PROGRESS` would have silently exempted every citation of itself — inside the entry annotated *"this entry is the class"*. The zero-scan residual said one check pads `scanned`; three do, and the worst returns a module-level constant that never touches the filesystem. The smoke check's "bounded twenty-second wait" terminated in under a second, because the banner it waited for is printed when the predicate is parsed, not when the stream attaches — the fix was shorter than the sleep it replaced. Its archive fallback took the *oldest* announcement in a five-minute window, which on a developer's simulator could query a dead isolate from a previous run. And a self-test figure has read **86** since cycle 1 while the runner said 88, laundered through five cycles by a `HISTORICAL` marker belonging to a different number in the same paragraph.

**What did converge is the arithmetic.** Every mechanically derivable figure re-derives at this head — suite counts, register tallies, migration and mapped-table counts, both self-test counts, the control tally, the trailer count. The fifth reviewer confirmed each independently. The dating of the trailer denominator to a fixed commit is the one fix in the closeout that survives the head moving.

**No sixth cycle was run, and the reason is not that the tree is clean.** The reviewer's closing sentence was that a sixth cycle should expect to find a seventh, and the evidence supports it. Running one more would produce another finding, another fix, and another head — and the verdict does not depend on the answer, because the compliance gate below is unmet independently of it. Stopping here and saying so is more honest than stopping at a cycle that happened to come back quiet.
...
|---|---|---|---|
| 1 | `341c479` | 2 BLOCKING, 5 HIGH | the tree, and the corpus |
| 2 | `d10f0df` | 3 HIGH | **all three inside the checks written to catch cycle 1** |
| 3 | `3cf1bb6` | 1 BLOCKING, 3 HIGH | **the blocker inside that commit's own headline act** |

Cycle 3's blocker: CI-020 was re-opened in the improvement log and left carrying the opposite status in **four** other live documents — two of which discharge a Phase 6 entry criterion on it. The re-opening was the most deliberate act in the commit, and it was applied to one document out of five.

**The reviewer named the single cause under three of its findings, and it is not a documentation problem.** *The corpus builds a rule for the instance in front of it and states the class.* `register-traceability` covered three of the four registers this project keeps — EV, KAR-CTL, KAR-RSK — and the continual-improvement log, which has ids and a status column and the same authority, had no rule at all. That is precisely where the blocker landed. The gate-outcome rule covered one sentence shape and the gate records' own house shape was not it. The zero-scan rule covers `scanned === 0` and one check pads past it.

**So the fix is the class, not the instance.** The improvement log is the fourth register now, and the seeded blocker fails, and the report names the entry, the status it found and the status the log holds. *(Paraphrased here rather than quoted: writing the seeded sentence verbatim makes the rule fire on the paragraph describing the rule, which it duly did.)*. The gate-outcome rule has self-test coverage for the first time — the fixture's gate records had no outcomes at all, so `recorded === undefined` short-circuited the whole rule and a reviewer deleted thirty-two lines of it with the suite green. Both shapes are seeded now, and neutering the rule fails two cases.

**The zero-scan residual is recorded rather than closed.** `scanned` is a number each check chooses, and test 25 counts the platform lifecycle document alongside the module ones — so an empty module window still reports `scanned: 1` and passes. That is the class in its current form: not a checker going quiet, but a checker able to describe its window in a way the rule cannot audit. KAR-RSK-044 stays OPEN for it.

**What the fourth cycle verified and could not break**, having tried: `statusForResult` is genuinely shared and load-bearing in both paths, and the cycle-1 headline `PASS test 16 (scanned: 0)` is now a FAIL with the N/A arm reachable and counted outside `passed`; all five new self-test guards fail when their rules are removed; the namespace test has a real mechanism behind it; and every figure in the delta re-derives — both self-test counts, the architecture summary, the control tally, the workspace and Flutter counts, EV-505's migration and Flutter figures.

**Two known limits are now written down rather than discovered.** The traceability rule does not match the plural copula (`are`), an adverb between verb and status (`is now DESIGNED`), a span containing a period or a file path, or the `(D)`/`(I)` framework shorthand; and excluding negators from the span silences a genuine fork that happens to contain the word `no`. The gate-outcome rule does not match the copula or colon forms. Each is a real gap, none is a regression, and stating them is the difference between a limit and a surprise.

**The reviewer's answer to the question it was asked — is the corpus stable? — was no.** Four cycles, each finding the next defect inside the last fix. That answer is load-bearing for this phase's verdict, and it is recorded here rather than argued with.
...
| F-1 | **KAR-RSK-042 closed on "both are now bounded" and there were THREE reads.** `PreferencesKeyValueStore.open` awaits `getAll` unbounded, and `bootstrapKararApp` awaits it BEFORE the bounded security-state open and before `runApp` — so a host that never answers leaves no widget tree at all. The class had no test in the tree | Fixed and tested. Four tests, including one that fails in sixteen seconds when the bound is removed — it bounds its own await and names the unbounded call rather than hanging, which is the better behaviour and not what the first description of it claimed. The risk row and the policy header both say three now, and say which one was missed |
...
**CI-020 was closed twice on less than its own criterion, and it is OPEN now.** It first closed on `tool/startup_smoke.sh` existing; nothing ran it — the `mobile-ios` lane built the bundle, read its `Info.plist`, and never started it, which is CI-006's pattern exactly. The lane was then wired to boot a simulator and require the check to pass, with no `continue-on-error`, and that is real: on a runner-created simulator it reports *settled within 46s, the UI isolate answered*. But the entry's stated verification is **two runtimes**, and only one is automated. A delta reviewer named the second closure as the same defect one revision later, and was right. The row stays OPEN with the iOS half recorded as delivered and the Android half carried as **CI-021**. **KAR-RSK-042 is not held open by it.** The risk is that the client does not start, and it starts on both runtimes with evidence on each; what remains unfinished is the automation of the second runtime, which is CI-020's subject and not the risk's.
...
| N5 | HIGH (verification finding, not a code defect introduced by this phase) | **The mobile client, executed on a runtime for the first time, did not reach a screen.** See below | **Fixed in part, retracted in part.** The unbounded platform I/O was real and is bounded, with 13 tests. The follow-on claim that the UI thread had stopped was measured on a wedged simulator and is withdrawn: the client reaches sign-in on two fresh runtimes and `tool/startup_smoke.sh` passes on both | KAR-RSK-042 CLOSED; CI-020 OPEN (its criterion is two automated runtimes and only iOS is automated) | — |
...
**What it changes is the standing of every client claim.** Before this session, "no device execution" recorded an absence of proof and nothing more. It now records something better: the surface Phase 5 built has been reached, by hand and by an automated check, on two runtimes. **KAR-RSK-042 is CLOSED; CI-020 is OPEN.** The risk was that the client does not start, and it starts on two runtimes with evidence on each — that is closed. CI-020's stated verification is an automated check passing on TWO runtimes, and only iOS is automated, so it is not. *(Four places in this corpus said both were closed. A delta reviewer found them: the re-opening had been applied to the improvement log and to one paragraph of the phase report, and not to the gate record or to either entry-criteria list — so a Phase 6 entry criterion was recorded as discharged on an entry that is open.)*, and the Phase 6 entry criterion they carried is discharged rather than carried forward.
