...
| Documentation (`pnpm docs:check`) | **15/15**, self-test ok over **35** cases across two fixtures, 293 markdown files scanned | close-out |
| Prisma mapping (`node scripts/db/prisma-mapping-check.mjs`) | **61 mapped tables match the live database** | checkpoint |
| Workspace (`pnpm test`) | **3153 passed / 12 skipped / 0 failed (3165 total)** across 222 files (221 passed, 1 skipped), on PostgreSQL 17.11, **identical under both server default timezones** and over three consecutive runs with zero orphan scratch databases | close-out |
