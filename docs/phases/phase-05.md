...
| Flutter surface | **Not built** — no feature folder, route, fixture or screen |
...
- **There is no Flutter financial surface.** `apps/mobile/lib/features/` holds ten folders and every one of them is account, identity or platform state. No feature folder, route, fixture or screen refers to an account, a transaction, a balance or an import, and none may be added until the capability is implemented, deployed and deliberately added to the navigable set — three separate acts, in that order.
