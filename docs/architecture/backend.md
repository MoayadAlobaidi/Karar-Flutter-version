...
**Since Phase 4 every problem document leaves through exactly one writer, and the media type is the boundary's to set.** Twenty-five operation/status pairs across five modules were returning correctly shaped RFC 7807 bodies under `application/json` while the contract declared `application/problem+json`: the media type was set only by the error boundary, which sees *thrown* failures, and those twenty-five answered by writing to the reply object directly. They now throw, and the boundary supplies the media type. A shared writer imported *by* the modules was not available — `apps/api` depends on every module, so the import would have been a project-reference cycle — which means the direction of the dependency decided the design rather than convenience. The deviation ledger now asserts **empty**, so a handler that answers a problem through the reply object again is a failing test rather than a silent entry.
...
**A failed resolution is not an empty one, and Phase 4 made that a type rather than a convention.** Every bootstrap enrichment port returns a tagged outcome, so `RESOLVED` with no capabilities is an answer and an unreadable store is a `503 BOOTSTRAP_UNAVAILABLE` carrying a retryable flag and no detail. One code covers all five dependency failures deliberately: naming which dependency failed would disclose topology. The failure handling is asymmetric on purpose — a jurisdiction failure short-circuits before the downstream resolvers run, a PolicyPack or capability failure fails the call, and an operating-entity failure degrades only its own section, because a client can render an account without a legal-entity summary but cannot render services without a resolution.
...
The health row is implemented as of Phase 2 and extended in Phase 4: `apps/api` `/readyz` executes real checks — `SELECT 1` on the application role, a read-only migration verify (applied == latest), and a round trip to the rate-limit store — and answers 503 with per-check states on any failure, states only, never hosts, ports, credentials or driver errors. The rate-limit check was added because several identity controls fail closed without that store, so a readiness answer that ignored it reported ready while login could not be served. The connection is established during startup rather than by the first request that needs it: the client is lazy and does not queue commands, so an unconnected first command was rejected outright and surfaced as a 503 on the first login after boot. The worker runs its own loopback-only health server (`KARAR_WORKER_PORT`, default 3001) whose `/readyz` additionally checks that the relay and poller loops are recently alive.
