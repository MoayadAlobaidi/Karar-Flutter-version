...
**Phase 5 has built a financial data foundation and nothing that reaches it.** Five modules — `financial-accounts`, `transactions`, `financial-connections`, `payment-instruments`, `transfer-matching` — carry schema, domain, ports, repositories and tests behind migrations `0087`-`0099`, under [ADR-0027](../adr/0027-calendar-day-and-instant.md) and [ADR-0028](../adr/0028-multi-rail-financial-sources.md), both ACCEPTED. Not built: manual entry as a running path, CSV ingestion, the categorization pipeline, any OpenAPI operation, any client method, any screen. The canonical `currentPhase` marker is therefore still **4**, and architecture test 24 (resource limits declared) is **deferred** — both move only with the first real mounted ingestion path, because a test that activates against an empty tree scans nothing and passes vacuously.

**No consumer product capability is implemented.** Every entry in the capability registry is `NOT_IMPLEMENTED` and deployed nowhere, no jurisdiction is approved, no PolicyPack is approved, and no capability is reachable anywhere — the financial foundation included, which is schema and code rather than a capability. The client renders that state honestly rather than hiding it: its authenticated home is an account and security surface whose services section is a stated empty state.

**No provider is connected and no real institution API exists.** No issuer named in the catalogue exposes an interface to Karar, no credential of any kind is stored anywhere, and thirteen acquisition rails are named of which only `MANUAL` and `USER_FILE_UPLOAD` may exist — enforced by a database CHECK, not by application code. Nothing in the product may render "Connected", "Synced" or "Linked" for data a person typed or uploaded. **The financial-data retention question is a legal decision and has not been taken**, so none of this data may reach a deployed environment: the only retention provider that exists is synthetic, labelled `SYNTHETIC_NO_LEGAL_EFFECT`, and refuses to construct outside LOCAL and TEST.
