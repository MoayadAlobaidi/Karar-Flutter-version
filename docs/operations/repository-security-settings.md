...
| Branch protection on `main`: required status checks | Enabled, "require branches to be up to date" on, with exactly these checks: `workspace`, `architecture`, `mobile`, `mobile-android`, `mobile-ios`, `codeql`, `secrets`, `dependency-review`, `iac-and-containers`, `sbom`, `mobile-supply-chain` | VERIFIED 2026-08-18 — all 11 required, strict up-to-date on. Added in two steps at the Phase 4 PR close by `MoayadAlobaidi`, the only account with admin: `mobile-android` and `mobile-ios` once both had run to success on PR #7 head `ebdcf99`, then `mobile-supply-chain` once it had run to success on **two** heads (`ebdcf99` and `de564066f9d39ff01c56fd2ce402a19c71d4b03f`), which is the job-name stability condition this document set for itself. Prior set was 10; final set is 11. `dependency-audit` and `licenses` stay report-only by design and are deliberately NOT required. Every prior check was preserved, and strict up-to-date, admin enforcement, pull-request-only merge, blocked force-push and blocked deletion were all re-read through the API afterwards and are unchanged **Re-verified 2026-08-18 after the Phase 4 merge:** the eleven required checks, strict up-to-date enforcement, admin binding, and the force-push and deletion blocks all survived the merge unchanged. On a push to `main`, `dependency-review` reports `skipped` — it is a pull-request-only action with no base to diff against, so a skip there is designed behaviour rather than a disabled control, and the branch summary must not be read as though a skip were a failure or a pass. | Engineering Owner | `gh api repos/{owner}/{repo}/branches/main/protection/required_status_checks` | EV-466, EV-467 |
...
- `mobile-supply-chain` (security.yml; added 2026-08-18 — carries the blocking mobile secret scan, which runs before Flutter is installed so a scan cannot be affected by what a package resolution pulled in). It was held back one step deliberately, until it had run to success on two PR heads (`ebdcf99` and `de564066f9d39ff01c56fd2ce402a19c71d4b03f`) rather than one; that condition is now met and the gap this document previously recorded is closed.
