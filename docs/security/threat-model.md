...
**These controls exist in the schema and in two modules; none of them has ever been exercised by a request, because no route reaches this data.** They are recorded now so that the gap between "implemented" and "exercised" is visible rather than discovered at the phase gate.
