...
**The CSV statement is the largest single piece of customer financial data this platform holds, and it is held as an object rather than as a column.** `statement_import_sources` carries the AEAD parameters per row — algorithm, key version, a 12-byte nonce and a 16-byte auth tag, each length-checked at the database — plus a SHA-256 integrity checksum over the **ciphertext**, and **no bytes of the statement itself**. The bytes live in an object store behind `EncryptedSourceStorePort`, which is also why erasure has two halves: the row cascades, and the object is deleted through the port, because no database cascade reaches a byte the database does not hold. The algorithm is stored per row rather than assumed globally, so an algorithm migration can still read old objects — "everything is AES-256-GCM" is true right up until it is not. **None of that metadata has a field on the HTTP surface**: not the locator, not the store kind, not the byte length, not the algorithm, key version, nonce, auth tag or checksum. Existence is reported as a boolean, because a handle is enough to ask a store for somebody's bank statement.
