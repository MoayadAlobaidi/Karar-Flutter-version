...
2. **AC-029 states a coverage, and the coverage is partial.** The suite validates **82 of the 128 declared operation/status pairs** — the mobile-consumed surface it was scoped to — and its own header enumerates the 46 it does not reach and the three inside the `/auth` fragment that are unreachable in process rather than skipped. Both former deviation ledgers assert empty as an exact `toEqual([])`, so a new prose-only operation or a new direct-reply problem is a failing test rather than a silent entry, and a source scan (`apps/api/src/errors/problem-media-type-exclusivity.test.ts`) holds the problem media type to a single writer across the pairs the runtime ledger does not reach. The suite is **infrastructure-gated** and skips entirely when PostgreSQL and Redis are unreachable, so the figure describes a run with infrastructure present rather than a property a bare checkout demonstrates. **A response on one of the 46 uncovered pairs can still drift from its contract, and would surface as a client failure rather than as a gate.**
