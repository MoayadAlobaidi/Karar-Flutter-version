...
**Status:** ACTIVE mapping view · **Owner:** Compliance Owner · **Version:** 0.1 · **Date:** 2026-08-15 · **Review:** every phase gate (mapping deltas per gate report §5)
...
Status shorthand: D = DESIGNED · I = IMPLEMENTED · I[C1] = implemented contingent on Phase-1 CI merge · DEF = DEFERRED(phase) · EXC = exception.
...
| CC6.1 | Logical access restricted by design | KAR-CTL-007 (D), 008 (D), 009 (D), 010 (DEF 3), 011 (DEF 3), 012 (DEF 13), 013 (DEF 8/20), 033 (D), 036 (D) | Layered model: `docs/security/access-control.md` |
| CC6.2 | Credentials issued/deprovisioned with authorization | KAR-CTL-007 (D), 014 (D), 013 (DEF) | Trivial population today; the review still happens |
| CC6.3 | Access modification/removal, least privilege | KAR-CTL-009 (D), 014 (D), 013 (DEF) | |
...
| CC6.7 | Data protected in movement and on media | KAR-CTL-032 (DEF 17+), 034 (DEF), 036 (D) | Classification matrix defines in-transit rules |
...
| CC7.1 | Vulnerabilities and misconfigurations detected | KAR-CTL-025 (D — scan runs, report-only), 026 (I[C1]), 027 (I[C1]), 029 (I[C1]), 020 (I[C1]) | The one family with real Phase-1 tooling; 025's blocking threshold is the open piece |
| CC7.2 | Anomalies monitored and evaluated | KAR-CTL-039 (DEF 2/8), 040 (DEF 2), 041 (DEF 20) | **Entirely deferred — nothing runs to monitor.** Honest gap |
...
| CC8.1 | Changes authorized, tested, approved, deployed controlled | KAR-CTL-015 (D), 016 (I[C1]), 017 (EXC), 018 (DEF 19), 019 (I), 020 (I[C1]), 031 (D) | EXC-001 is the material qualification on "approved" |
...
| TSC | Theme | Karar controls (status) |
|---|---|---|
| PI1.1 | Data/processing definitions support correct outputs | KAR-CTL-020 (I[C1]) — no-float money-path test; 021 (D) |
| PI1.2–PI1.3 | Inputs and processing complete, accurate, timely | Deferred to the financial engine (Phase 6: `VerifiedFinancialFacts`, exhaustive calculator tests) and ingestion resource limits (threat model T7, architecture test 24) |
...
Processing-integrity evidence begins to exist at Phases 5–6; today's contribution is structural only.

## Confidentiality

| TSC | Theme | Karar controls (status) |
|---|---|---|
| C1.1 | Confidential information identified and protected | KAR-CTL-033 (D), 034 (DEF), 012 (DEF 13), 038 (I), 040 (DEF 2) |
| C1.2 | Confidential information disposed of when no longer needed | KAR-CTL-037 (DEF 5), 050 (DEF 5/16) |
...
1. **CC7.2 (monitoring) is empty until Phase 2/8/20** — deliberate, phase-gated, and recorded as KAR-RSK-003.
2. **CC8.1's approval leg rests on EXC-001** — the mechanical gates are real; independent human approval is not, yet.
3. **A1/PI evidence cannot exist before Phases 5–6/17–20** — the observation window starts after those, not before.
