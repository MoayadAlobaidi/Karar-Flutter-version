...
| `flutter_secure_storage_platform_interface` | The federated platform interface behind `flutter_secure_storage` | Declared for the same reason as the row above and under the same rule: `FlutterSecureStore` is the seam a non-completing keychain call hangs at, and there was no test of the real adapter at all until one was written. A fake platform registered here is the only way a host test can make the keychain misbehave. **Dev-only**, and it must move with `flutter_secure_storage` | **None performed** |
| `vm_service` | The Dart VM service client used by `apps/mobile/tool/startup_probe.dart` | **Dev-only, and it never enters a shipped artifact** — it is a tool that asks a running debug build's UI isolate what it is showing, which is what `tool/startup_smoke.sh` decides on. Recorded because rule 1 is about first use and not about blast radius, and because a package that is dev-only today is one pubspec line away from not being | **None performed** |

**Three of these are DEV-ONLY** — the two federated platform interfaces and `vm_service` — and are declared directly rather than reached transitively, because a test or a tool that reaches into a transitive package is depending on it whether or not the pubspec admits it. They are in this table rather than in a footnote because rule 1 is about first use, but the distinction matters and hiding it would overstate the shipped supply chain by three.

Also pinned and recorded, without their own rows because they carry no security decision: `intl`, `cupertino_icons`, and `meta`.

**The honest summary of this table:** **six** suppliers entered a customer-facing artifact before anyone reviewed any of them, and **three more** were adopted as dev-only dependencies on the same unreviewed basis — nine rows, none reviewed, under a rule (rule 1) that requires the review *before first use*. *(This sentence said seven and covered one table. It was written when the table held seven rows and the two platform interfaces had not both been declared; `flutter_secure_storage_platform_interface` was added during the Phase 5 closeout for the keychain adapter's first real test, and `vm_service` for the startup probe. The count is re-derived from the rows rather than carried.)* Pinning them exactly proves that what builds is what was read; it says nothing about whether what was read is safe. Two of the seven — the token store and the authenticator — are the two that would matter most if it were not.
