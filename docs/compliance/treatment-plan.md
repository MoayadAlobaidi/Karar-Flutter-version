...
**Status:** ACTIVE register · **Owner:** Security Owner · **Version:** 0.1 · **Date:** 2026-08-15 · **Review:** every phase gate
...
| KAR-RSK-003 no monitoring | Nothing pre-runtime by design; observability foundations Phase 2; projections/ops views Phase 8; alert routing, severity model, on-call rotation as Phase 20 gates (KAR-CTL-041) | 2, 8, 17–20 | Phase 20 gate passes with alerts live and rotation staffed |
...
| KAR-RSK-009 sealed key custody | Rotation designed in at Phase 2; custody strategy + recovery policy + canary designed at Phase 13 with the vault; approved strategy, rehearsed recovery, running canary as Phase 20 gates before any production `SEALED` data (ADR-0017) | 2, 13, 20 | Phase 20 custody gates pass |
| KAR-RSK-010 documentation drift | Docs checks in CI (023) from Phase 1; assurance-claim linkage test 26 from Phase 2; per-gate reconciliation of claims vs. built reality (phase-compliance-gate.md item 6) | 1, 2, per-phase | Never fully — held at bay per gate; score drops when test 26 operates |
