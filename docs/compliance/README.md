...
**Nothing.** Karar holds no SOC 2 report, no ISO/IEC 27001 certificate, no regulatory approval, and claims none (Assurance Claim Registry AC-009). No control in this directory is asserted as OPERATING or EVIDENCED. At Phase 1 there is no application, no cloud, no customer, and no production environment; what exists is a documented control set, three registers with honest contents, and CI tooling whose first evidence is still pending. Any document or conversation representing more than that is wrong and should be corrected against this page.
...
The **[control matrix](control-matrix.md)** is the core: 50 controls with honest statuses. Everything else either feeds it (risks, exceptions, policies) or views it (SOC 2 mapping, ISO SoA). Statuses live in the matrix only — the framework views must never fork them.
