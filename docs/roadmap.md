...
| **5** | Financial data platform — institutions, connectors, accounts, transactions, normalization, dedup, provenance, categorization; **manual + CSV `IMPLEMENTED`**; **erasure strategies enforced** — **IN PROGRESS** ([report](phases/phase-05.md)); the account and transaction data foundation exists behind migrations 0087-0093 and is reachable by nothing — manual entry, CSV ingestion, categorization as a running path, and any API or client surface are **not built**, and retention is unresolved | +erasure |
