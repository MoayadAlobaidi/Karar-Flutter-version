...
| **5** | Financial data platform — institutions, connectors, accounts, transactions, normalization, dedup, provenance, categorization; **manual + CSV `IMPLEMENTED`**; **erasure strategies enforced** — **IN PROGRESS** ([report](phases/phase-05.md)); six financial modules plus `provider-capabilities` behind migrations 0087-0101, reached by **27 operations over 21 `/financial/*` paths**, with manual entry and CSV statement import both running and `currentPhase` moved to 5 alongside architecture test 24. The Flutter surface was built later in the phase — seven feature folders, every route capability-gated, the system document picker implemented on both platforms, deterministic categorisation running on both write paths, and internal transfer suggestions generated by the platform. The closeout verification executed the client for the first time, found two unbounded storage reads ahead of the network, bounded them, and now reaches the sign-in screen on both an Android emulator and an iOS simulator with `tool/startup_smoke.sh` passing on each (KAR-RSK-042, closed). Still open: **the statement-import picker is unreachable end to end by anyone today**, because the surface is gated on a capability nothing deploys (KAR-RSK-043); **no build has run on a physical device**; no provider connector; nothing deployed; retention unresolved; and account deletion deliberately not exposed over HTTP | +erasure |
...
| Retention decision — minimums, maximums, and whether they differ for an original statement file versus derived transactions. **Reached its phase at the Phase 5 gate and was NOT taken: no legal review has occurred and no period is invented anywhere in this repository.** It blocks the Phase 5 **policy** close and any deployed environment holding financial data; it does not block the Phase 5 engineering close, and the gate states both answers rather than one. Residual meanwhile: the only retention provider is `SYNTHETIC_NO_LEGAL_EFFECT` and fails closed outside LOCAL and TEST, so a deployment with no approved decision refuses to compose the financial surface at boot | Phase 5 policy — **due, unmet** | Legal + regulator |
