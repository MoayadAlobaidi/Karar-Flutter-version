...
| **3.5** | **Jurisdiction & Capability Foundation** — Country/Jurisdiction, PolicyPack `qa/v1`, resolution-strategy registry, **`SubjectPolicySelection` mechanism**, capability registry, availability model, entitlements, **session tenant binding + bootstrap** — **in progress** ([report](phases/phase-03-5.md)) | `ADDED` |
