...
- **The navigable-capability set is empty**, correctly — but no longer for the reason this line used to give. `TRANSACTIONS` IS implemented; nothing is deployed and no PolicyPack clears anything, so nothing is navigable.
