...
void main() {
  group('the listing', () {
    testInBothDirections('renders every transaction with its amount, direction and booking day', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      final formatter = KararFormatter.of(tester.element(find.byType(TransactionsScreen)));
      expect(find.text(formatter.applyNumerals('2026-03-01')), findsWidgets);
    }, textScales: featureTextScales);

    testInBothDirections('two currencies are rendered as two figures and never combined', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testWidgets('a filter narrows the listing through the platform, not locally', (
      ...
      final repository = ScriptedTransactionsRepository(transactions: someTransactions());
      await pumpFeatureScreen(
        tester,
        const TransactionsScreen(),
        surfaceSize: wideSurface,
    ...
    testWidgets('the platform decides whether there is another page', (WidgetTester tester) async {
    ...
    testInBothDirections('an empty listing distinguishes "none yet" from "none matching"', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
  ...
  group('the detail screen', () {
    ...
    testInBothDirections('the history is rendered oldest first with its attribution', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      final l10n = l10nOf(tester, TransactionDetailScreen);
      final formatter = KararFormatter.of(tester.element(find.byType(TransactionDetailScreen)));

      expect(find.text(formatter.applyNumerals(l10n.transactionRevisionNumber(1))), findsWidgets);
    ...
    testInBothDirections('provenance names the rail, the mapping and the algorithm versions', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testWidgets('the active category and who decided it are shown', (WidgetTester tester) async {
      await pumpDetail(tester, detail: transactionDetail(activeCategory: categoryAssignment()));
      final l10n = l10nOf(tester, TransactionDetailScreen);
  ...
  group('recording a transaction by hand', () {
    ...
    testInBothDirections('the amount is a magnitude and the direction is chosen separately', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('a malformed day is refused with a validation summary', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      expect(repository.created, isEmpty);
      expect(find.text(l10n.transactionFormValidationSummaryTitle), findsOneWidget);
    }, textScales: featureTextScales);

    testWidgets('a day typed in Arabic numerals is understood', (WidgetTester tester) async {
    ...
    testWidgets('with no accounts the form says to add one first', (WidgetTester tester) async {
      await pumpFeatureScreen(
        tester,
        const ManualTransactionScreen(),
        surfaceSize: wideSurface,
  ...
  group('correcting a transaction', () {
    ...
    testInBothDirections('says that a correction is appended rather than overwriting', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testWidgets('sends the expected version and only what changed', (WidgetTester tester) async {
    ...
    testWidgets('the magnitude and the direction travel together', (WidgetTester tester) async {
      final repository = await pumpCorrection(tester);
      final l10n = l10nOf(tester, TransactionCorrectionScreen);
  ...
  group('the correction use case', () {
    ...
    test('a manual draft with a signed magnitude is refused before it is sent', () async {
      final repository = ScriptedTransactionsRepository();
      final result = await RecordManualTransaction(repository)(
        ManualTransactionDraft(
          accountId: 'account-0001',
          entry: MoneyEntry(magnitude: money('-100'), direction: MoneyDirection.moneyOut),
