...
import 'package:karar_mobile/features/financial_accounts/domain/page.dart' as paging;
...
ScriptedAccountsRepository walletRepository() => ScriptedAccountsRepository(
  ...
  balances: <String, List<BalanceSnapshot>>{
    walletId: <BalanceSnapshot>[balance(accountId: walletId, amount: money('75000'))],
  },
...
ScriptedInstrumentsRepository twoVirtualCards() =>
    ScriptedInstrumentsRepository(<String, List<PaymentInstrument>>{
...
List<String> textsUnder(WidgetTester tester, Finder finder) => <String>[
  for (final element in find.descendant(of: finder, matching: find.byType(Text)).evaluate())
    if ((element.widget as Text).data != null) (element.widget as Text).data!,
];

void main() {
  group('two virtual cards on one wallet', () {
    testInBothDirections('render as two instruments and one balance', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      expect(amounts, hasLength(1), reason: 'a wallet with two cards has ONE balance:\n$amounts');
    }, textScales: featureTextScales);

    testInBothDirections('NO FIGURE APPEARS INSIDE AN INSTRUMENT ROW', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      for (var index = 0; index < 2; index++) {
        ...
        for (final rendered in textsUnder(tester, row)) {
          ...
          expect(
            ...
            isFalse,
            reason:
                'an instrument row rendered "$rendered", which reads as '
    ...
    testInBothDirections('the section states that a card holds no balance of its own', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      await pumpWallet(tester, locale: locale, textScale: scale);
      expect(find.text(mountedL10n(tester).instrumentsNoBalanceNotice), findsOneWidget);
    }, textScales: featureTextScales);
  ...
  group('an instrument states its type, mask and status', () {
    testInBothDirections('every field the contract sends is rendered', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      Finder inSection(String text) =>
          find.descendant(of: find.byType(AccountInstrumentsSection), matching: find.text(text));
    ...
    testWidgets('a mask that could be a full number is withheld', (WidgetTester tester) async {
      await pumpFeatureScreen(
        tester,
        const AccountDetailScreen(accountId: walletId),
        surfaceSize: detailSurface,
    ...
    testWidgets('no instrument row claims a live issuer link', (WidgetTester tester) async {
      await pumpWallet(tester);
      for (final claim in <Pattern>[RegExp('connect', caseSensitive: false), RegExp('متصل')]) {
  ...
  group('an account with no instruments', () {
    testInBothDirections('says so, and still states that a card holds no balance', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
  ...
  group('the instrument contract, decoded', () {
    ...
    ({ApiPaymentInstrumentsRepository repository, FakeApiTransport transport}) repositoryFor(
      Object? body,
    ) {
    ...
    test('every instrument type maps, and an unknown one is unrecognised', () async {
      Future<InstrumentType> typeFor(String wire) async =>
          (await instrumentFrom(instrumentBody(instrumentType: wire))).instrumentType;
    ...
    test('every status maps, and spendable is stated rather than derived', () async {
      ...
      final held = await instrumentFrom(instrumentBody(status: 'ACTIVE', spendable: false));
    ...
    test('the issuer link claim is read from the wire and never inferred', () async {
      expect((await instrumentFrom(instrumentBody())).impliesLiveIssuerLink, isFalse);
    });
    ...
    test('the request goes to the account it is nested under', () async {
      final held = repositoryFor(<String, Object?>{
        'items': <Object?>[],
        'page': <String, Object?>{'limit': 50, 'returned': 0, 'hasMore': false, 'nextCursor': null},
      });
