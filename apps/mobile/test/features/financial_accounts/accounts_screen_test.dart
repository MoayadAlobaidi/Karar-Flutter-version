...
void main() {
  group('the portfolio renders the whole holding', () {
    testInBothDirections('every account, wallet and cash row is present', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('two accounts of the same type and currency at one issuer stay two rows', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('a wallet shows its subtype and a non-wallet does not', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('each row states its own currency code', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('the safe mask is rendered and nothing longer is', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('a long issuer and a long account name do not clip', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      await pumpPortfolio(tester, repository: repository, locale: locale, textScale: scale);
  ...
  group('balances are shown by kind and never combined', () {
    testInBothDirections('each reported kind gets its own labelled figure', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('the notice that nothing is added up is always present with figures', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('two currencies produce the per-currency notice, never a total', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('THE MONEY ON SCREEN IS EXACTLY WHAT THE SOURCES REPORTED', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      expect(
        ...
        hasLength(3),
        reason:
            'three figures were reported — booked and available on one '
      ...
      expect(amounts.where((String amount) => amount.contains('QAR')), hasLength(2));
  ...
  group('source labels are honest', () {
    testInBothDirections('a manual account reads as manually added and a CSV one as imported', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('nothing on the portfolio reads as a connection', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
  ...
  group('grouping and filtering', () {
    testInBothDirections('the group-by control offers every axis and switches the headings', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      expect(
        find.text(locale.languageCode == 'ar' ? issuerOneNameAr : issuerOneNameEn),
        findsWidgets,
      );
    ...
    testWidgets('clearing the filters restores the whole portfolio', (WidgetTester tester) async {
      await pumpPortfolio(tester);
      final l10n = mountedL10n(tester);
  ...
  group('states', () {
    testInBothDirections('an empty portfolio invites the person to add one', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
    ...
    testInBothDirections('a failed read renders an error state with a retry', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      await pumpPortfolio(
        tester,
        repository: ScriptedAccountsRepository(listFailure: const DependencyUnavailableFailure()),
        locale: locale,
        textScale: scale,
      );
      final l10n = mountedL10n(tester);
    ...
    testInBothDirections('Arabic renders right to left', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
