...
void main() {
  group('status badge', () {
    testInBothDirections('always carries a word and an icon, never colour alone', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
  ...
  group('banner', () {
    testInBothDirections('announces itself and offers its action', (
      ...
      expect(tester.getSemantics(find.byType(KararBanner)), isSemantics(isLiveRegion: true));
    ...
    testInBothDirections('the dismiss control is named in the active language', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      final AppLocalizations l10n = AppLocalizations.of(tester.element(find.byType(KararBanner)));
  ...
  group('state views', () {
    testInBothDirections('the empty state uses localized default copy', (
      ...
      await pumpKarar(tester, const KararStateView.empty(), locale: locale, textScale: scale);
    ...
    testWidgets('the Arabic empty state renders Arabic copy', (WidgetTester tester) async {
      await pumpKarar(tester, const KararStateView.empty(), locale: KararLocalization.arabic);
      expect(
    ...
    testInBothDirections('the error state announces itself and offers a retry', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      await pumpKarar(
        tester,
        KararStateView.error(detail: 'Reference b7f1c2', onAction: () => retries++),
        locale: locale,
        textScale: scale,
      );
      final AppLocalizations l10n = AppLocalizations.of(
      ...
      expect(tester.getSemantics(find.byType(KararStateView)), isSemantics(isLiveRegion: true));
  ...
  group('loading', () {
    testInBothDirections('the indicator is announced as a live region', (
      ...
      final SemanticsHandle handle = tester.ensureSemantics();
      await pumpKarar(tester, const KararLoadingIndicator(), locale: locale, settle: false);
      final AppLocalizations l10n = AppLocalizations.of(
    ...
    testInBothDirections('a named subject is spoken', (
      ...
      expect(find.bySemanticsLabel(l10n.stateLoadingWithSubject('Karar')), findsOneWidget);
      handle.dispose();
    });
    ...
    testInBothDirections('an inline indicator stays silent', (
      ...
      final SemanticsHandle handle = tester.ensureSemantics();
      await pumpKarar(tester, const KararLoadingIndicator.inline(), locale: locale, settle: false);
      expect(
        find.descendant(
  ...
  group('card', () {
    ...
    testInBothDirections('a pressable card is a named button', (
      ...
      expect(
        ...
        isSemantics(label: 'Device security', isButton: true, hasTapAction: true),
      );
  ...
  group('checkbox tile', () {
    testInBothDirections('reports its checked state and toggles from anywhere in the row', (
      WidgetTester tester,
      Locale locale,
      double scale,
    ) async {
      ...
      expect(tester.getSemantics(find.byType(KararCheckboxTile)), isSemantics(isChecked: true));
      handle.dispose();
    }, textScales: testTextScales);
    ...
    testInBothDirections('the checkbox row meets the touch target floor', (
      ...
      await pumpKarar(
        tester,
        KararCheckboxTile(label: 'I have read the terms', value: false, onChanged: (_) {}),
        locale: locale,
      );
      expect(
