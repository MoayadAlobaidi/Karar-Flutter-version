...
final class ProblemFailureMapper {
  ...
  Failure map({
    ...
    return _mapStatus(statusCode, code: code, problem: problem, correlationId: joined);
  }

  Failure? _mapCode(String code, {ProblemDetails? problem, String? correlationId}) =>
      switch (code) {
        ApiErrorCode.authenticationRequired =>
          AuthenticationRequiredFailure(code: code, correlationId: correlationId),
        ApiErrorCode.sessionExpired => SessionExpiredFailure(
            code: code,
            correlationId: correlationId,
            reason: SessionEndReason.expired,
          ),
        ApiErrorCode.notAuthorized || ApiErrorCode.membershipRequired => NotAuthorizedFailure(
            code: code,
            correlationId: correlationId,
        ...
        ApiErrorCode.consentRequired =>
          ConsentRequiredFailure(code: code, correlationId: correlationId, purposeRef: problem?.reason),
        ApiErrorCode.reConsentRequired =>
          ReConsentRequiredFailure(code: code, correlationId: correlationId, purposeRef: problem?.reason),
        // A tenant binding is the remedy for both codes, and the startup
        // machine routes them to the same destination.
        ApiErrorCode.tenantSelectionRequired || ApiErrorCode.tenantBindingRequired =>
          TenantSelectionRequiredFailure(code: code, correlationId: correlationId),
        ...
        ApiErrorCode.capabilityResolutionUnavailable =>
          CapabilityResolutionUnavailableFailure(code: code, correlationId: correlationId),
        ...
        ApiErrorCode.rateLimited =>
          RateLimitedFailure(code: code, correlationId: correlationId, retryAfter: problem?.retryAfter),
        ...
        ApiErrorCode.invalidStatusTransition =>
          ConflictFailure(code: code, correlationId: correlationId, reason: problem?.reason),
        ...
        ApiErrorCode.tenantNotFound =>
          NotFoundFailure(code: code, correlationId: correlationId),
  ...
  Failure _mapStatus(
    ...
    String? correlationId,
      ...
      switch (statusCode) {
        ...
        401 => AuthenticationRequiredFailure(code: code, correlationId: correlationId),
        ...
        409 => ConflictFailure(code: code, correlationId: correlationId, reason: problem?.reason),
