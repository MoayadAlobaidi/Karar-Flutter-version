/**
 * OpenTelemetry SDK initialization — the ONLY place SDK/exporter packages are
 * touched. Everything else (platform and modules) emits through
 * `@opentelemetry/api`, so telemetry stays provider-neutral: the OTLP stream
 * goes to whatever collector the deployment profile points at
 * (docs/architecture/backend.md §11). Locally that is the compose
 * otel-collector on 127.0.0.1:4318.
 */
